...
# Purpose: Combine text(7) + tone(4→7) into a final 7-class distribution.
# Inputs:
#   - text_json:  {"utterances":[{speaker,text,start_time,end_time,emotions:[{label,score}×7]}]}
#   - tone_json:  {"utterances":[{speaker,start_time,end_time,emotions4:[{label,score}×4],confidence:float,qc:{...}}]}
# Output:
#   List[dict]: {speaker, text, start_time, end_time, emotions:[{"label","score"}×7]} (scores in %), sorted desc
#
# CHANGE: 7 emotions incl. "neutral"; audio mapping contributes to neutral.
# CHANGE: Calls only to speaker_state: detect_change / adjust_emotions / update (no logic here).

from __future__ import annotations
from typing import Any, Dict, List

from .labels import EMOTIONS_7, EMOTIONS_4, list_to_vec, to_api_top1, vec_to_list_pct
