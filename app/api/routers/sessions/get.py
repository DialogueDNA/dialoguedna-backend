...
from ...mappers.session_mapper import to_sessions_response, to_session_response
...
def list_sessions(
    ...
    try:
        return to_sessions_response(facade.get_sessions(user_id=ctx.id))
...
def get_session(
        ...
        ctx: UserContext = Depends(require_user)):
    try:
        return to_session_response(facade.get_session(session_id=session_id, user_id=ctx.id))
    except Exception as e:
        raise HTTPException(status_code=404, detail=str(e))
