...
@router.get("/{session_id}/emotions/rebuild", response_model=AnalyzedEmotionsResponse ,summary="Get per-segment emotions (text/audio/fused)")
def get_emotions(
        session_id: str,
        facade: ApplicationFacade = Depends(get_facade),
        ctx: UserContext = Depends(require_user)):
    try:
        return to_analyzed_emotions_response(facade.get_analyzed_emotions_view(facade.rebuild_emotions(session_id=session_id, user_id=ctx.id)))
