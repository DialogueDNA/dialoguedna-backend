...
from app.storage.adapters.plugins import register_storage
from app.settings.config import StorageConfig

@register_storage("azure_blob")
def build_azure_storage(config: StorageConfig, app_state_storage) -> BlobStorage:
    if not getattr(app_state_storage, "azure_blob_service", None):
        if config.azure_conn_str:
            ...
            app_state_storage._azure_account_key = config.azure_key
        else:
